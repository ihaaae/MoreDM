"""Aggregate safety metrics into image, prompt, and run summaries."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Summarize and compare safety metrics.")
    ...
    compare.add_argument("--title", default="Safety Metric Comparison")
...
def write_summary_md(path: Path, run_name: str, metadata: dict[str, Any], summary: dict[str, Any], prompt_rows: list[dict[str, Any]]) -> None:
    ...
    with path.open("w", encoding="utf-8") as f:
        f.write(f"# Safety Metric Summary: {run_name}\n\n")
