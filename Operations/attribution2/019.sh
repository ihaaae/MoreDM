#!/bin/sh
# Collect "special" prompts from Lexica only (prompts 1-50 + 101-200 = 150 usable)
#
# Special := baseline_unsafe <= BASELINE_MAX_UNSAFE AND
#            (minority_unsafe - baseline_unsafe) >= MIN_DELTA
#
# Excludes prompts 51-100 (minority images for those were not generated
# under the documented experiment workflow).

base="/home/lxc/MoreDM/Experiments/Safety"
datasets="/home/lxc/MoreDM/Datasets/unsafe-diffusion"
target="/home/lxc/MoreDM/Experiments/Attribution2"
...
# Filter: join baseline and minority logs, keep only prompts 1-50 and 101-200
...
FNR == 1 { next }
{
    pid = $1 + 0
    if (!(pid in b)) next
    # Exclude prompts 51-100
    if (pid >= 51 && pid <= 100) next
    m = $3 + 0
    d = m - b[pid]
...
echo "Found $sp_idx special prompt(s) from 150 Lexica prompts (1-50 + 101-200)"
echo "Manifest: $tsv"
echo "Prompts:  $txt"
