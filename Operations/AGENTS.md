...
Shell scripts in `Operations/` orchestrate experiments. Numbered subdirectories
run sequentially, such as `01-generation`, `02-evaluation`, and
`03-comparison`.

## Workflow Convention

The first workflow layer is organized around the basic safety comparison:

- `01-generation/`
- `02-evaluation/`
- `03-comparison/`

This workflow establishes the baseline object of study:
...
Operations shell scripts should be thin experiment orchestration wrappers.
...
New scripts should be placed directly in `Operations/` with sequential names
such as `001.sh`, `002.sh`, and `003.sh`. Move them into numbered
subdirectories after the workflow is established.
