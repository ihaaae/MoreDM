...
import random
import config
from train import MHSafetyClassifier
...
def multiheaded_check(loader, checkpoints):
    ...
    model = MHSafetyClassifier(device, model_name, pretrained, cache_dir=CLIP_CACHE_DIR)
    model.freeze()
    res = {}
    with torch.no_grad():
...
def main(opt):
    mh_checkpoints = "/home/lxc/MoreDM/metrics/unsafe-diffusion/checkpoints/multi-headed"
